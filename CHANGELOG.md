...
- **18 new unit tests** for Express middleware and Fastify adapter (express.spec.ts, fastify.spec.ts)
- **5 new E2E tests** (prototype pollution, depth limit, content-type variants, blacklist)
- **Clear README badge**: "3.1x vs jsonc-parser" — now references the actual alternative being compared
- **Improved README performance section**: explains what each implementation validates and how the speed comparison relates to features provided

### Fixed

- **SonarQube S4790** (Security Hotspot): replaced MD5 with DJB2 hash in `buildCacheKey` — DJB2 is a fast non-cryptographic hash designed for hash tables, making the security context unambiguous
...
- **Coverage**: Express middleware coverage from 14% to 76%
- **Flaky performance test**: increased tolerance in `fast-path.spec.ts` from 2x to 3x
...
- **Removed Buffer roundtrip** for string inputs: `Buffer.from(input).toString()` eliminated from hot path (10-21% faster)
...
[1.1.4]: https://github.com/pas7-studio/nestjs-strict-json/releases/tag/v1.1.4
...
- Resolved all 17 SonarCloud S5797 issues — added missing assertions to tests in `streaming-parser.spec.ts` and `fast-path.spec.ts`

### Tests

- All 558 tests passing (79 streaming + 479 other)
- TypeScript typecheck clean

[1.0.1]: https://github.com/pas7-studio/nestjs-strict-json/releases/tag/v1.0.1
