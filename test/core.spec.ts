...
describe("parseStrictJson", () => {
  ...
  it("throws on duplicate key", () => {
    expect(() => parseStrictJson(Buffer.from('{"flag":true,"flag":false}'))).toThrow(DuplicateKeyError)
  })

  it("throws on nested duplicate key", () => {
    expect(() => parseStrictJson(Buffer.from('{"a":{"b":1,"b":2}}'))).toThrow(DuplicateKeyError)
  })
  ...
  it("throws on invalid JSON", () => {
    expect(() => parseStrictJson(Buffer.from('{invalid json}'))).toThrow("Invalid JSON")
  })
  ...
  it("allows JSON within maxBodySizeBytes", () => {
    expect(parseStrictJson(Buffer.from('{"a":1}'), { maxBodySizeBytes: 100 })).toEqual({ a: 1 })
  })
  ...
  it("handles nested objects without duplicates", () => {
    expect(parseStrictJson(Buffer.from('{"a":{"b":1},"c":2}'))).toEqual({ a: { b: 1 }, c: 2 })
  })

  it("handles deep nested duplicate key", () => {
    expect(() => parseStrictJson(Buffer.from('{"a":{"b":{"c":1,"c":2}}}'))).toThrow(DuplicateKeyError)
  })
})
